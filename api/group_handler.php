...
$traefik_host_id = !empty($_POST['traefik_host_id']) ? (int)$_POST['traefik_host_id'] : null;
$is_edit = !empty($id);
...
try {
    ...
    if ($is_edit) {
        if ($id == 1 && strtolower($name) !== 'general') { // Prevent renaming the default 'General' group
            throw new Exception('Cannot rename the default "General" group.');
        }
        $stmt = $conn->prepare("UPDATE `groups` SET name = ?, description = ?, traefik_host_id = ? WHERE id = ?");
        $stmt->bind_param("ssii", $name, $description, $traefik_host_id, $id);
        if (!$stmt->execute()) throw new Exception('Failed to update group: ' . $stmt->error);
        $group_id = $id;
        $log_action = 'Group Edited';
        $log_details = "Group '{$name}' (ID: {$id}) has been updated.";
        $success_message = 'Group successfully updated.';
    } else {
        $stmt = $conn->prepare("INSERT INTO `groups` (name, description, traefik_host_id) VALUES (?, ?, ?)");
        $stmt->bind_param("ssi", $name, $description, $traefik_host_id);
        if (!$stmt->execute()) throw new Exception('Failed to create group: ' . $stmt->error);
