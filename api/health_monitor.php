...
try {
    // --- PART 1: Service-level Health Checks (existing logic) ---
        ...
        if ($new_status === 'unhealthy' && $service['status'] !== 'unhealthy') {
            ...
            try {
                ...
                if ($is_swarm) {
                    ...
                    $target_container = null;
                    foreach ($containers as $container) {
                        if (($container['Labels']['com.docker.compose.project'] ?? null) === $service['stack_name']) {
                            $dockerClient->restartContainer($container['Id']);
                            $action_message = "Restart command sent for container '{$container['Names'][0]}'.";
                            break;
                        }
                    }
    ...
    // --- PART 2: Container-level Health Checks (new logic) ---
    if ($global_health_check_enabled) {
        echo "\n--- Memulai Pengecekan Kesehatan Kontainer Global ---\n";
        
        // 1. Get all Docker hosts
        ...
        foreach ($all_hosts as $host) {
            ...
            try {
                ...
                foreach ($containers as $container) {
                    ...
                    // 2. Inspect container to see if it has a health check configured
                    $details = $dockerClient->inspectContainer($container_id);
                    if (!isset($details['State']['Health'])) {
                        continue; // Skip containers without a health check
                    }

                    echo "  -> Mengevaluasi kontainer: '{$container_name}'\n";

                    // 3. Get current status from our DB
                    $stmt_status = $conn->prepare("SELECT * FROM container_health_status WHERE container_id = ?");
                    $stmt_status->bind_param("s", $container_id);
                    $stmt_status->execute();
                    $current_status_record = $stmt_status->get_result()->fetch_assoc();
                    $stmt_status->close();

                    // 4. Get health status from Docker
                    $docker_health_status = $details['State']['Health']['Status'] ?? 'unknown';
                    $is_healthy = ($docker_health_status === 'healthy');
                    $log_message = "Container health status: {$docker_health_status}";
                    echo "    -> Hasil: {$log_message}\n";

                    // 5. Update status in the database (using thresholds from a default service for now, can be made configurable)
                    // For simplicity, we'll use a hardcoded threshold here.
                    $healthy_threshold = 2;
                    $unhealthy_threshold = 3;

                    $new_status = $current_status_record['status'] ?? 'unknown';
                    $failures = $current_status_record['consecutive_failures'] ?? 0;
                    $successes = $current_status_record['consecutive_successes'] ?? 0;
                    ...
                    $stmt_update = $conn->prepare(
                        "INSERT INTO container_health_status (container_id, host_id, status, consecutive_failures, consecutive_successes, last_checked_at, last_log)
                         VALUES (?, ?, ?, ?, ?, NOW(), ?)
                         ON DUPLICATE KEY UPDATE 
                            status = VALUES(status), 
                            consecutive_failures = VALUES(consecutive_failures), 
                    ...
                    $stmt_update->bind_param("sisiss", $container_id, $host['id'], $new_status, $failures, $successes, $log_message);
                    $stmt_update->execute();
                    $stmt_update->close();

                    // 6. Trigger auto-healing if status changed to 'unhealthy'
                    if ($new_status === 'unhealthy' && ($current_status_record['status'] ?? '') !== 'unhealthy') {
                        ...
                        try {
                            ...
                            $action_message = "Restart command sent for container '{$container_name}'.";
                            echo "    -> SUKSES: {$action_message}\n";
                            log_activity('SYSTEM', 'Auto-Healing Action', $action_message);
                        } catch (Exception $e) {
                            echo "    -> ERROR saat auto-healing kontainer: " . $e->getMessage() . "\n";
                            log_activity('SYSTEM', 'Auto-Healing Error', "Failed to restart container '{$container_name}': " . $e->getMessage());
                        }
                    }
                }
            } catch (Exception $e) {
