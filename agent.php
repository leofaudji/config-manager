...
/**
 * SELF-UPDATE LOGIC
 * Checks for a new agent version from the Config Manager and updates itself.
 */
function run_self_update_check() {
    global $configManagerUrl, $apiKey;

    echo "[SELF-UPDATE] Checking for new agent version (current: " . AGENT_VERSION . ")...\n";

    // 1. Get the latest version from the server
    $version_url = rtrim($configManagerUrl, '/') . '/api/agent/version';
    $ch_version = curl_init($version_url);
    curl_setopt($ch_version, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch_version, CURLOPT_TIMEOUT, 5);
    $version_response = curl_exec($ch_version);
    curl_close($ch_version);

    $latest_version_data = json_decode($version_response, true);
    $latest_version = $latest_version_data['version'] ?? null;

    if (!$latest_version) {
        echo "[SELF-UPDATE] Could not retrieve latest version from server. Skipping update.\n";
        return;
    }

    // 2. Compare versions
    if (version_compare($latest_version, AGENT_VERSION, '>')) {
        echo "[SELF-UPDATE] New version available: {$latest_version}. Attempting to download...\n";

        // 3. Download the new script
        $download_url = rtrim($configManagerUrl, '/') . '/api/agent/download';
        $new_script_path = __FILE__ . '.tmp';

        $fp = fopen($new_script_path, 'w+');
        $ch_download = curl_init($download_url);
        curl_setopt($ch_download, CURLOPT_HTTPHEADER, ['X-API-Key: ' . $apiKey]);
        curl_setopt($ch_download, CURLOPT_TIMEOUT, 30);
        curl_setopt($ch_download, CURLOPT_FILE, $fp); // Write to file
        curl_exec($ch_download);
        $http_code = curl_getinfo($ch_download, CURLINFO_HTTP_CODE);
        curl_close($ch_download);
        fclose($fp);

        if ($http_code === 200 && filesize($new_script_path) > 0) {
            // 4. Replace the old script with the new one
            rename($new_script_path, __FILE__);
            echo "[SELF-UPDATE] Update successful. The agent container will now restart to apply changes.\n";
        ...
        echo "[SELF-UPDATE] Agent is up to date.\n";
    }
}
...
function run_check_cycle() {
    ...
    // --- NEW: Get all network drivers once for efficiency ---
    $network_id_to_driver_map = [];
    try {
        $all_networks_on_host = $dockerClient->listNetworks();
        foreach ($all_networks_on_host as $network) {
            $network_id_to_driver_map[$network['Id']] = $network['Driver'] ?? 'unknown';
        }
    } catch (Exception $e) {
        log_message("  -> WARN: Could not list Docker networks, macvlan check may be affected: " . $e->getMessage());
    }
    ...
    foreach ($containers as $container) {
        try {
            ...
            // --- FIX: Move special exclusions to the top of the loop ---
            // These containers should be completely ignored by the agent to prevent
            // any processing attempts (stats, health checks, etc.) which might fail
            // due to their special nature (e.g., privileged, utility).
            $excluded_containers = [
                'cm-health-agent', // The agent itself
                'falco-sensor',    // Security agent
                'falcosidekick',   // Security agent sidekick
                'host-cpu-reader'  // Utility container
            ];

            if (in_array($container_name, $excluded_containers)) {
                log_message("    - Skipping excluded container '{$container_name}'.");
                continue; // Immediately move to the next container
            }
            ...
            if ($docker_health_status) {
                ...
                $check_flow_log[] = ['step' => 'Docker Healthcheck', 'status' => 'skipped', 'message' => 'No built-in HEALTHCHECK found. Proceeding to fallback checks.'];

                // --- RESTRUCTURED FALLBACK LOGIC ---
                // First, determine the network driver. Macvlan requires a special, direct check.
                $network_id = isset($details['NetworkSettings']['Networks']) ? key($details['NetworkSettings']['Networks']) : null;
                $network_driver = ($network_id && isset($network_id_to_driver_map[$network_id])) ? $network_id_to_driver_map[$network_id] : 'unknown';

                // --- Alur 2a: Logika Khusus untuk Standalone (termasuk macvlan) ---
                if (!$is_swarm_node) {
                    // Dapatkan IP dan Port Internal untuk pengecekan
                    $internal_ip = null;
                    $internal_port = null;
                    if ($network_id) {
                        $internal_ip = $details['NetworkSettings']['Networks'][$network_id]['IPAddress'] ?? null;
                    }
                    $tcp_ports = [];
                    ...
                    // Jika ini adalah macvlan, langsung gunakan nsenter.
                    if ($network_driver === 'macvlan') {
                        if ($internal_port) {
                            $container_pid = $details['State']['Pid'] ?? 0;
                            if ($container_pid > 0) {
                                log_message("    -> [MACVLAN] Checking TCP port {$internal_port} via nsenter on PID {$container_pid}.");
                                $command = "nsenter -t {$container_pid} -n -- nc -z -w 2 localhost " . escapeshellarg($internal_port);
                                exec($command, $output, $return_var);
                            } else {
                                log_message("    -> [MACVLAN] WARN: Could not get PID for container '{$container_name}'. Cannot use nsenter.");
                                $return_var = 1; // Force failure
                            }
                            if ($return_var === 0) {
                                $is_healthy = true;
                                $check_flow_log[] = ['step' => 'Internal TCP Check (nsenter)', 'status' => 'success', 'message' => "Connection to localhost:{$internal_port} inside the container's network namespace was successful."];
                            } else {
                                $is_healthy = false;
                                $check_flow_log[] = ['step' => 'Internal TCP Check (nsenter)', 'status' => 'fail', 'message' => "Connection to localhost:{$internal_port} inside the container's network namespace failed."];
                            }
                        } else {
                             $check_flow_log[] = ['step' => 'Internal TCP Check (nsenter)', 'status' => 'skipped', 'message' => 'No internal port could be determined for macvlan check.'];
                        }
                    }
                }

                // --- Alur 2b: Pengecekan Published Port (Hanya jika belum sehat dan BUKAN macvlan) ---
                if ($is_healthy === null && $network_driver !== 'macvlan') {
                    $all_published_ports = [];
                    ...
                    if (!empty($all_published_ports)) {
                        log_message("    -> Trying published ports on host.docker.internal: " . implode(', ', $all_published_ports));
                        ...
                        $check_flow_log[] = ['step' => 'Published Port Check', 'status' => 'skipped', 'message' => 'No published ports found to check.'];
                    }
                }

                // --- Alur 2c: Pengecekan Internal untuk Jaringan Bridge (Hanya jika belum sehat) ---
                if ($is_healthy === null && !$is_swarm_node && $network_driver === 'bridge') {
                    if ($internal_ip && $internal_port) {
                            log_message("    -> [Standalone/Bridge] Checking internal TCP port {$internal_ip}:{$internal_port}.");
                            $network_id_to_join = $network_id; // FIX: Define the network to join
...
try {
    run_self_update_check(); // Jalankan pemeriksaan pembaruan di awal
    run_check_cycle();
} catch (Exception $e) {
